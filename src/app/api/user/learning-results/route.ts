...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check cache first
    // const cached = await redis.get(`learning_results:${userId}`);
    ...
    const response: LearningResultsResponse = {
      success: true,
      user_id: userId,
    ...
    // await redis.set(`learning_results:${userId}`, JSON.stringify(response), 'EX', 300);

    console.log(`[Learning Results API] User ${userId}: ${facilityWeights.length} weights, ${recentDecisions.length} recent decisions`);

    return NextResponse.json(response);
  } catch (error) {
    console.error('[Learning Results API] Error:', error);
    return NextResponse.json({
      success: false,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Recalculate scores for specified facility types or all
    const typesToRecalculate = facility_types || [];

    if (typesToRecalculate.length > 0) {
      // Recalculate specific facility types
      for (const facilityType of typesToRecalculate) {
        ...
        if (weights) {
          const newScore = calculateWeightedScore(
            ...
            Math.round(weights.positive_feedback_score * 20),  // Approximate
      ...
      // Recalculate all weights for user
    ...
    console.error('[Learning Results API] Recalculation error:', error);
    return NextResponse.json({
      success: false,
